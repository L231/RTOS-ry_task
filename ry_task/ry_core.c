                 ...
                 ry_cpu_t    stack_size)
{
	...
	/* 配置任务栈 */
	stack_addr           = (void *)RY_ALIGN(stack_addr, RY_ALIGN_SIZE);
	stack_size           = RY_ALIGN_DOWN(stack_size, RY_ALIGN_SIZE);
	task->stack_addr     = stack_addr;
	task->stack_size     = stack_size;
#if RY_STACK_GROW_UP == 1
	task->sp             = ry_stack_init(func, param,
	                       (void *)((ry_cpu_t)stack_addr));
#else
	task->sp             = ry_stack_init(func, param,
	                       (void *)((ry_cpu_t)stack_addr+stack_size-4));
#endif
												 ...
												 ry_cpu_t    stack_size)
{
	ry_u8_t *stack_addr;
	ry_task_t *Task;
	stack_size  = RY_ALIGN(stack_size, RY_ALIGN_SIZE);
