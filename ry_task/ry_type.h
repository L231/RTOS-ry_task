...
#if RY_CPU_BIT_WIDE == RY_CPU_32BIT
typedef signed   int                    ry_int32_t;
#elif RY_CPU_BIT_WIDE == RY_CPU_16BIT
typedef signed   long int               ry_int32_t;
#endif
...
#elif RY_CPU_BIT_WIDE == RY_CPU_16BIT
typedef unsigned long int               ry_u32_t;
#endif
typedef unsigned int                    ry_cpu_t;

/* 向上对齐，返回的数可能更大 */
#define  RY_ALIGN(size, align)       (((ry_cpu_t)(size) + (align) - 1) & ~((align) - 1))
/* 向下对齐，返回的数可能更小 */
#define  RY_ALIGN_DOWN(size, align)  ((ry_cpu_t)(size) & ~((align) - 1))
...
#define  _STRING_(s)                    #s
#define  RY_STRING(s)                  _STRING_(s)
#define  _CONS_(s1, s2)                 s1##s2
#define  RY_CONS(s1, s2)               _CONS_(s1, s2)
