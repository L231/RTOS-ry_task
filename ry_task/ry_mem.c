...
{
    ry_mem_head_t *Mem;
    ryMem.heap_begin = RY_ALIGN((ry_u32_t)ryMem.heap, RY_ALIGN_SIZE);
    ryMem.heap_end   = RY_ALIGN_DOWN((ry_u32_t)(ryMem.heap + RY_HEAP_SIZE), RY_ALIGN_SIZE);
            ...
            if(Mem->size >= size)
            {
                if(Mem->size >= (size + MEM_STRUCT_ALIGN + MEM_MIN_ALIGN))
                {
                    ...
                    if(((ry_cpu_t)Mem + Mem->size + MEM_STRUCT_ALIGN) < ryMem.heap_end)
                        pMemHead_t((ry_cpu_t)Mem + Mem->size + MEM_STRUCT_ALIGN)->prev = Free;
                    /* 当前分割的内存配置大小、使用标志 */
...
void ry_free(void *mem)
{
    ...
    if(nFree != ryMem.heap_end && !(pMemHead_t(nFree)->size & RY_MEM_USED_FLAG))
    {
        ...
        pMemHead_t(nFree + pMemHead_t(nFree)->size + MEM_STRUCT_ALIGN)->prev = Free;
    ...
    if(Free != ryMem.heap_begin && !(pMemHead_t(pFree)->size & RY_MEM_USED_FLAG))
    {
        ...
        if(nFree != ryMem.heap_end)
            pMemHead_t(Free + pMemHead_t(Free)->size + MEM_STRUCT_ALIGN)->prev = pFree;
 ...
 *
 **/
void ry_mempool_reg(ry_mempool_t *mem, void *start, ry_u32_t size, ry_u32_t block_size)
{
    ry_u8_t *ptr, *ptrEnd;
    mem->begin          = (ry_u8_t *)RY_ALIGN((ry_u32_t)start, RY_ALIGN_SIZE);
 ...
 *
 **/
ry_mempool_t *ry_mempool_create(ry_u32_t block_num, ry_u32_t block_size)
{
    ry_u32_t size;
    ry_mempool_t *mem;
